...
一个会话绑定一个项目，项目的 `home` 说它的主目录在哪台机器的哪个目录；项目还可以在别的机器上有**副本**
（`workspaces`），那台机器上的 agent 就在副本里干活。
...
一个项目是**一件要做的事及其规矩**：Agent 怎么改它（直接改主目录，或在隔离副本里改完再合并回来），以及数据等级。它落在机器上的目录叫
