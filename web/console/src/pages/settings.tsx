...
function SettingRow({ path, field, view, value, disabled, restart, onChange }: { path: SettingPath; field: SettingsField; view: HubSettings; value: string; disabled: boolean; restart: ReactNode; onChange: (value: string) => void }) {
    ...
    return <div className="settings-field" data-setting={path}>
        <div><label className="settings-field-label" htmlFor={id}>{label}</label>{path.startsWith("gateway.") && <p className="settings-field-description">{t(`settingsPage.${path as Extract<SettingPath, `gateway.${string}`>}.hint`)}</p>}{(saved !== effective || edited) && <p className="settings-field-description settings-difference">{t("settingsPage.saved")}: <span data-desired>{format(saved)}</span>{saved !== effective && <> · {t("settingsPage.effective")}: <span data-effective>{format(effective)}</span>{field.apply_mode !== "live" && <> · {restart}</>}</>}</p>}</div>
...
function ChannelForm({ view, draft, disabled, onChange }: { view: ChannelSettings; draft: ChannelDraft; disabled: boolean; onChange: (value: ChannelDraft) => void }) {
    ...
    const field = (name: "app_id" | "owner_open_id" | "allowed_senders" | "blocked_senders") => <div className="settings-field"><div><label className="settings-field-label" htmlFor={`channel-${name}`}>{t(`settingsPage.channel.${name}`)}</label>{name.endsWith("senders") && <p className="settings-field-description">{t("settingsPage.senderHint")}</p>}</div>{name.endsWith("senders") ? <TextArea id={`channel-${name}`} aria-label={t(`settingsPage.channel.${name}`)} value={draft[name]} onChange={(value) => update({ [name]: value })} rows={3} isDisabled={disabled} /> : <Input id={`channel-${name}`} aria-label={t(`settingsPage.channel.${name}`)} value={draft[name]} onChange={(value) => update({ [name]: value })} size="sm" isDisabled={disabled} autoComplete="off" />}</div>;
    return <>
