...
export function FleetPage() {
    ...
    return (
        <div className="workbench-page flex min-w-0 flex-col">
            ...
            {refusedBehind.length > 0 && <div role="status" className="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg bg-warning-primary px-3 py-2 text-sm text-warning-primary"><span className="min-w-0 flex-1">{(refusedBehind.length === 1 ? tr("fleet.protocolBehindHintOne", { version: snap.hub.version || "—" }) : tr("fleet.protocolBehindHint", { count: refusedBehind.length, version: snap.hub.version || "—" }))}</span><Button size="sm" color="secondary" onClick={() => setUpgrading(refusedBehind)}>{tr("fleet.upgradeAll")}</Button></div>}
