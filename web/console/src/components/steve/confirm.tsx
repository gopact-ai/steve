...
// undone: what will happen, in the owner's words, and a single button to
// agree. The tone follows the act — red for what destroys, plain for what
// merely closes. A refusal from the server is shown here rather than
// somewhere else on the page, because this is where the owner is looking.
export function ConfirmDialog({ title, body, confirmLabel, tone = "destructive", onConfirm, onClose }: { title: string; body: ReactNode; confirmLabel: string; tone?: "destructive" | "primary"; onConfirm: () => Promise<void>; onClose: () => void }) {
    const { t: tr } = useI18n();
