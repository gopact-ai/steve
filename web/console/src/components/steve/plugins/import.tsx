    ...
    return <PluginSheet title={t("plugins.import")} dirty={source.location !== ""} busy={busy} onClose={onClose}>
        <form className="flex min-w-0 flex-col gap-4" onSubmit={(event) => { event.preventDefault(); void (preview ? save() : inspect()); }}>
            <fieldset disabled={busy || pending !== null} className="flex min-w-0 flex-col gap-4">
                ...
                <Input size="sm" name="plugin-source" autoComplete="off" spellCheck="false" label={t("plugins.location")} hint={t("plugins.sourceHint")} value={source.location} onChange={(location) => update({ ...source, location })} isRequired />
                {source.kind === "git" ? <><Input size="sm" name="plugin-commit" autoComplete="off" spellCheck="false" label={t("plugins.commit")} value={source.commit ?? ""} onChange={(commit) => update({ ...source, commit })} isRequired /><Input size="sm" name="plugin-subdir" autoComplete="off" spellCheck="false" label={t("plugins.subdir")} value={source.subdir ?? ""} onChange={(subdir) => update({ ...source, subdir })} /></> : null}
