    ...
    return <PluginSheet title={t("plugins.configure")} dirty={dirty} busy={busy} onClose={onClose}>
        <form className="flex min-w-0 flex-col gap-5" onSubmit={(event) => { event.preventDefault(); void save(); }}>
            <fieldset disabled={busy} className="flex min-w-0 flex-col gap-4">
                <Input size="sm" name="installation-id" autoComplete="off" spellCheck="false" label={t("plugins.installationId")} hint={t("plugins.installationHint")} value={id} onChange={setId} isRequired isDisabled={!!item} />
