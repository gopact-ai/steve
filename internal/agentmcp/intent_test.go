...
func TestUnknownOutcomeBlocksTheSameCallFromTheNextAttempt(t *testing.T) {
	...
	s.Anchor("oc_a", "oc_a", "om_a")

	// Attempt 1: the platform times out — outcome unknown.
	sender.timeout = true
	args := map[string]any{"content": "## milestone\nphase 1"}
	if text, isError := callTool(t, s.URL(), "tok-a", "feishu_send", args); !isError {
	...
	attempts.live = "att-2"
	text, isError := callTool(t, s.URL(), "tok-a", "feishu_send", args)
	...
	if text, isError := callTool(t, s.URL(), "tok-a", "feishu_send", map[string]any{"content": "## other\nnews"}); isError {
	...
	if text, isError := callTool(t, s.URL(), "tok-a", "feishu_send", args); isError || !strings.Contains(text, "sent message_id=") {
