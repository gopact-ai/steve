...
func (b peerSSHBackend) UpgradeTarget(ctx context.Context, nodeID string) (sshconnect.UpgradeTarget, error) {
	if nodeID == b.peer.Config.NodeID {
		return sshconnect.UpgradeTarget{}, errors.New(b.peer.text.For(ctx).T(i18n.ClusterUpgradeSelf))
	}
	alias, err := b.MachineAlias(ctx, nodeID)
	if err != nil {
...
func (b peerSSHBackend) MachineAlias(ctx context.Context, nodeID string) (string, error) {
	text := b.peer.text.For(ctx)
	if nodeID == b.peer.Config.NodeID {
		return "", errors.New(text.T(i18n.ClusterBrowseSelf))
	}
	link, ok := b.peer.link(nodeID)
	if !ok {
		return "", errors.New(text.T(i18n.ClusterNoTunnel))
	}
...
func (p *Peer) askBuild(nodeID string) func(context.Context) (string, error) {
	return func(ctx context.Context) (string, error) {
		...
		if runtime == nil {
			return "", errors.New(p.text.For(ctx).T(i18n.ClusterServiceNotRunning))
		}
		member, ok := runtime.Status().Members[nodeID]
		if !ok {
			return "", errors.New(p.text.For(ctx).T(i18n.ClusterNotMember))
		}
...
func awaitBuildWithin(ctx context.Context, ask func(context.Context) (string, error), askLimit, interval time.Duration, version string) error {
	...
	for {
		...
		case timedOut && said != "timeout":
			said = "timeout"
			sshconnect.Report(ctx, text.T(i18n.ClusterAskAgain))
		case !timedOut && err.Error() != said:
			said = err.Error()
			sshconnect.Report(ctx, text.T(i18n.ClusterNoAnswerYet, said))
		}
		select {
		case <-ctx.Done():
			switch {
			case seen != "":
				return errors.New(text.T(i18n.ClusterStillVersion, seen))
			case answered:
				return errors.New(text.T(i18n.ClusterStillOldProgram))
			}
			return errors.New(text.T(i18n.ClusterNoReanswer))
		case <-ticker.C:
		}
	}
}
