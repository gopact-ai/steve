...
func TestResumePrefersAttachedAuthorizedAttempt(t *testing.T) {
	...
	plan := ResumePlan(i18n.New(i18n.LocaleZH), ResumeRequest{Source: source, Live: &LiveAttempt{Source: source, Attached: true, AuthorityValid: true, Cursors: Cursors{InputAccepted: 5, OutputPublished: 12}}})
...
func TestResumeNeverTreatsTimeoutOrEpochAloneAsWriterIsolation(t *testing.T) {
	...
	for _, evidence := range []IsolationEvidence{
		...
		plan := ResumePlan(i18n.New(i18n.LocaleZH), req)
...
func TestResumeChecksPostCheckpointExternalActionsAndCapabilities(t *testing.T) {
	...
	if plan := ResumePlan(i18n.New(i18n.LocaleZH), req); plan.Action != ResumeAskUser || plan.Question.Code != "actions-not-reconciled" {
	...
	if plan := ResumePlan(i18n.New(i18n.LocaleZH), req); plan.Action != ResumeAskUser || plan.Question.Code != "action-result-unknown" {
...
func TestResumeRejectsWrongCheckpointOrStaleTarget(t *testing.T) {
	...
	if plan := ResumePlan(i18n.New(i18n.LocaleZH), req); plan.Action != ResumeAskUser || plan.Question.Code != "checkpoint-mismatch" {
		t.Fatalf("unrelated session checkpoint reused: %+v", plan)
	}
	req = safeResumeRequest(t)
	req.Target.ExecutionEpoch = req.Source.ExecutionEpoch
	if plan := ResumePlan(i18n.New(i18n.LocaleZH), req); plan.Action != ResumeAskUser {
