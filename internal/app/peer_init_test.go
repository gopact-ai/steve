...
func TestPeerInitCommandRejectsArgumentsBeforeWriting(t *testing.T) {
	...
	for _, args := range [][]string{
		{"--state-dir", ""},
		{"--state-dir", " "},
		{"--state-dir", root, "extra"},
		{"--state-dir", root, "--config", "existing.json"},
		{"--config", "existing.json", "--state-dir", root},
		{"--state-dir", root, "--storage-level", "restricted"},
		{"--state-dir", root, "--node-id", "node"},
		{"--state-dir", root, "--raft-address", "127.0.0.1:0"},
		{"--state-dir", root, "--peer-address", "127.0.0.1:0"},
		{"--state-dir", root, "--ui-address", "127.0.0.1:0"},
	} {
		var out, diagnostic bytes.Buffer
		if err := runInitClusterCommand(args, &out, &diagnostic); err == nil || out.Len() != 0 {
...
func TestPeerInitCommandPreservesExistingServiceConfiguration(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "service.json")
	cfg := &config.Config{
		Gateway: config.Gateway{
			HubID: "service", OwnerID: "owner", StatePath: filepath.Join(root, "state", "state.json"),
			ReadModelAddr: "127.0.0.1:0", ReadModelToken: strings.Repeat("fixture", 8),
		},
		Projects:  map[string]config.Project{"workspace": {Home: config.ProjectHome{Path: root}}},
		Harnesses: map[string]config.Harness{"mock": {Command: "mock"}},
		Agents:    map[string]config.Agent{"worker": {Harness: "mock", Default: true}},
	}
	...
	var out, diagnostic bytes.Buffer
	if err := runInitClusterCommand([]string{"--config", path, "--storage-level", "restricted"}, &out, &diagnostic); err != nil {
		t.Fatal(err)
	}
	var result struct {
	...
	if err := json.Unmarshal(out.Bytes(), &result); err != nil || result.ClusterID != "service" || result.Config != cluster.DefaultClusterConfigPath(path) {
		t.Fatalf("existing service result: %+v %v", result, err)
	}
	after, err := os.ReadFile(path)
	if err != nil || !bytes.Equal(before, after) {
		t.Fatalf("initialization changed the service configuration: %v", err)
	}
}
...
func TestServicePeerInitializationDeploysPluginThroughCoordinator(t *testing.T) {
	...
	var out, diagnostic bytes.Buffer
	if err := runInitClusterCommand([]string{"--state-dir", root}, &out, &diagnostic); err != nil {
		t.Fatal(err)
	}
