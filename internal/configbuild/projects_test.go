...
func controllerFixture(t *testing.T) (*config.Config, string, ProjectController, *ledger.Ledger) {
	t.Helper()
	cfg := &config.Config{Agents: map[string]config.Agent{"main": {Harness: "mock", Default: true}}, Harnesses: map[string]config.Harness{"mock": {Command: "mock"}}, Projects: map[string]config.Project{"p": {Home: config.ProjectHome{Path: "/project"}}}, Gateway: config.Gateway{HomePath: "/steve-home", DefaultProject: "p"}}
...
func TestProjectCandidateDoesNotPublishBeforeFileCommit(t *testing.T) {
	...
	candidate.Projects["bad"] = config.Project{Home: config.ProjectHome{Path: "/project/nested"}}
	writes := 0
...
func TestCommittedProjectFailureRecoversFromFileAfterRestart(t *testing.T) {
	...
	err := controller.Commit(t.Context(), candidate, func() error {
		err := config.Save(path, candidate)
	...
	if _, ok, _ := restarted.Store.Get(t.Context(), config.ReservedHomeProject); !ok {
...
func TestProjectCommitReconcilesAfterDirectorySyncWarning(t *testing.T) {
	...
	candidate.Projects["new"] = config.Project{Home: config.ProjectHome{Path: "/new"}}
	err := controller.Commit(t.Context(), candidate, func() error {
		// A saved file whose directory could not be synced.
		err := config.Save(path, candidate)
		if err == nil {
			err = &config.CommittedError{Err: errors.New("cannot sync parent")}
		}
		if err == nil || config.Committed(err) {
			*cfg = *candidate
		}
...
func TestCloneIntentSurvivesUnappliedFile(t *testing.T) {
	cfg, path, controller, _ := controllerFixture(t)
	cfg.Nodes = map[string]config.Node{"remote": {Addr: "127.0.0.1:1", Token: "test"}}
	p := cfg.Projects["p"]
	p.Workspaces = []config.ProjectWorkspace{{Node: "remote", Path: "/copy", Origin: "cloned", Source: "repository"}}
	cfg.Projects["p"] = p
	if err := config.Save(path, cfg); err != nil {
		t.Fatal(err)
	}
