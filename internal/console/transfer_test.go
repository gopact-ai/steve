...
func TestProjectTransferIsScopedDurableAndIdempotent(t *testing.T) {
	...
	_ = source.Save(raw)
	exported, err := ExportProjectDocument(source, "a", []string{"console:mixed"})
	if err != nil {
		t.Fatal(err)
	}
	...
	target := &memDoc{}
	if err := ImportProjectDocument(target, exported); err != nil {
		t.Fatal(err)
	}
...
func TestProjectTransferRejectsUnattributableHistory(t *testing.T) {
	...
	_ = doc.Save(raw)
	if _, err := ExportProjectDocument(doc, "a", []string{"console:a"}); err == nil {
