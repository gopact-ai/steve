...
var (
	_ contextProvider         = turntest.IdleCoordinator{}
	_ conversationInitializer = turntest.IdleCoordinator{}
	_ inputParser             = turntest.IdleCoordinator{}
	_ localizedVerbLister     = turntest.IdleCoordinator{}
	_ sessionResetter         = turntest.IdleCoordinator{}
	_ setupProvider           = turntest.IdleCoordinator{}
	_ suggester               = turntest.IdleCoordinator{}
)

// plainHandler runs turns and offers nothing else.
type plainHandler struct{}

func (plainHandler) Handle(context.Context, turn.Request) (turn.Result, error) {
...
// The idle coordinator answers the way a handler without those
// capabilities is answered for.
func TestConsoleOverACoordinatorThatKnowsNothing(t *testing.T) {
	for name, h := range map[string]Handler{"plain": plainHandler{}, "idle": turntest.IdleCoordinator{}} {
