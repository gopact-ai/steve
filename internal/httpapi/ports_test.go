...
func TestCapabilityRoutesWithoutTheirService(t *testing.T) {
	...
	for _, route := range []struct {
		...
		{"GET", "/console/nodes/worker/agents", "", 501, jsonStored, `{"error":"This service does not support registering node tools"}`},
		{"POST", "/console/nodes/worker/agents", `{}`, 501, jsonStored, `{"error":"This service does not support registering node tools"}`},
		{"GET", "/console/ssh/candidates", "", 501, jsonStored, `{"error":"SSH onboarding is not enabled"}`},
		{"POST", "/console/coordination/transfer", `{}`, 501, "Content-Type: application/json", `{"error":"This service has not enabled coordination state replication"}`},
