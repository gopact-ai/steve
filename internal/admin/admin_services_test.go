...
func TestHubRestartReceiptPrecedesStopAndSurvivesReadiness(t *testing.T) {
	...
	first, err := NewServices(admin, execution.New(context.Background(), nil), gate, func() { stopped.Add(1) }, doc)
	if err != nil {
		t.Fatal(err)
	}
	...
	next, err := NewServices(admin, execution.New(context.Background(), nil), gate, func() { stopped.Add(1) }, doc)
	if err != nil {
		t.Fatal(err)
	}
