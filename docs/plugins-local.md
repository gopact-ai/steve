...
节点的插件包协议（属于节点协议 v2 基线）目前提供 prepare、inspect 和凭据元数据查询；它校验部署的项目范围、目标节点、包摘要、普通配置、凭据版本和平台/解释器需求。内部 `Selection` 固定会话的部署集合，通过节点的插件运行时准备并绑定实际运行目录。`pluginledger.Library` 可把包内容按项目复制到独立节点，并从同一账本记录恢复；本地 CLI 的独立缓存不自动加入该库。
