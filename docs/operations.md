...
侧栏底部“设置”的通用页可选择简体中文、English 或跟随浏览器，并设置外观。显式选择保存在当前浏览器，不写入 Hub 配置；界面切换保留草稿、标签页和阅读状态。普通 API 请求带 `Accept-Language`，一次提交的 locale 在接收时固定，重试沿用原提交身份与语言。回复与拒绝使用请求的语言；请求未指明 Steve 支持的语言时使用 Hub 默认语言。没有具体请求者的内容——后台任务、跨节点转发、广播通知，以及启动和 `steve doctor` 经语言目录输出的说明——使用 Hub 默认语言，按写入时的语言保存，切换语言不会重新翻译已保存的文字。通过 SSH 接入或升级机器时，安装记录按发起者的语言写入；SSH 隧道状态、协作内容副本的修复通知和节点之间的互联核对使用 Hub 默认语言；新接入的机器沿用发起接入的 Hub 的默认语言，远端的 `steve link` 按该 SSH 账户的 `LANG` 输出。机群中的 Hub 默认语言是设置页保存在共享配置里的那一项，修改后节点随即采用；应用尚未在某个节点上运行过时，该节点使用自己配置文件中的语言。用户输入、材料、Agent 输出和历史正文不会随界面切换被重写；浏览器切换语言也不修改 Hub 默认语言或飞书连接配置。
