...
启用集群的服务可以从已连接机器导入 Codex、Claude Code、Grok 和 dsh 的历史会话。先在来源机器登记对应工具和 Agent，并添加与历史会话原目录一致的项目；然后打开会话侧栏的“导入历史会话”，选择 Agent、查找历史、选择会话和项目，点击“导入并打开会话”。默认查找该机器运行用户的 `~/.codex`、`~/.claude`、`~/.grok` 或 `~/.dsh`，Botmux 等使用独立 home 时可以填写那个绝对路径。来源工具适配器必须声明支持 ACP `session/load` 或 `session/resume`。

目标项目的写入权限、绑定及原工作目录会先于历史复制检查。单次快照最多 256 MiB、10,000 个文件；每个节点的 `native-imports` 与 `native-runtimes` 各限制新建准入为 128 项、8 GiB，崩溃留下的临时目录也计入。达到限制会拒绝新增迁移，已有导入收据仍可重试；运行后的历史增长仍由工具本身管理。关闭会话后保留历史，便于继续迁移；清理前需在服务停止且确认会话不再使用后归档相应记录，Steve 不自动删除用户上下文。
...
执行节点还须声明 `native_resume.v1` 能力；混合版本集群中的旧节点会明确提示更新 `steve-node`，不会接收可能丢失上下文的续接请求。
