...
验收通过后，在主任务详情点击“完成任务” / “Complete task”，或发送 `/tasks complete ID`（别名 `done`、`完成`）。省略编号选择当前会话最新的聊天主任务；重复完成同一编号返回稳定的已完成确认。完成不会重置会话或 native context；下一条新消息打开新主任务，旧 continuation 和 resume 不能重开已关闭任务。
