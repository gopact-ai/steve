...
升级顺序：v1 与 v2 互不连通，协调节点和执行节点都升级完之前，两者之间的连接一直被拒绝。桌面 App 集群由本机协调时先升级 App，再从控制台升级经 SSH 加入的机器；协调职责已交接给其他机器时，先升级当前协调节点，或先把协调交接回本机。服务端部署把协调节点的 `steve` 与各执行节点的 `steve-node` 一起替换并重启。
