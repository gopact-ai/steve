...
| `coordination: unavailable: this replica has heard from no consensus leader for ...` | 协调节点超过 ApplyTimeout（默认 5 秒）没有听到任何共识 leader，放弃当前业务代。重新听到 leader 后，先经一次仲裁读确认自己仍是协调者，再重新激活。持续出现时检查该节点到 leader 的 Raft 连接。 |
