...
历史目录发生变化时需要重新查找后选择；重复提交同一次导入会打开原来的 Steve 会话。恢复报错会保留错误与来源，不会自动创建空会话代替旧上下文。当前导入保留来源机器和原工作目录，尚不执行跨机器目录改写；源会话应先结束当前一轮再导入。单次历史快照上限为 256 MiB、10,000 个文件，不接收链接或特殊文件。读取列表和导入接口需要控制台所有者认证。未启用节点会话的节点，或所在平台（非 Unix）无法保存原生历史的节点，会拒绝读取和导入并说明原因。
