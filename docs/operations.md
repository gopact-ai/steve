...
| `coordination: unavailable: this replica stopped leading consensus and knows no other leader` | 协调节点同时是共识 leader，失去多数派后在 leader 租约到期时下台，又不知道新的 leader，立即放弃当前业务代：多数派可能已经选出别的 leader 并任命了别的协调者。重新听到 leader 后，先经一次仲裁读确认自己仍是协调者，再重新激活。leader 身份直接交给一个本机能联系上的成员时不放弃业务代。持续出现时检查该节点到其他成员的 Raft 连接。 |
