...
- 传入时读取 `gateway.read_model_addr` / `gateway.read_model_token`，token 为空且连接地址是 loopback（精确的 `localhost` 或 loopback IP）时读取 `gateway.state_path` 所在目录的 `loopback-token`（Hub 尚未启动过则为空），其他地址不读取该文件；存在 `<config>.cluster.json` 时，地址改用 sidecar 的 `ui_address`，token 仍来自原配置。普通 `gateway` 的空地址使用原默认地址，其监听地址可省略 `http://`，`0.0.0.0`、`[::]` 和省略主机的 `:端口` 分别转为 `127.0.0.1`、`[::1]` 和 `127.0.0.1`。
...
历史目录发生变化时需要重新查找后选择；重复提交同一次导入会打开原来的 Steve 会话。恢复报错会保留错误与来源，不会自动创建空会话代替旧上下文。当前导入保留来源机器和原工作目录，尚不执行跨机器目录改写；源会话应先结束当前一轮再导入。单次历史快照上限为 256 MiB、10,000 个文件，不接收链接或特殊文件。读取列表和导入接口需要控制台所有者认证，旧版节点会提示升级。
