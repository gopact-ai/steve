...
`peer-init` 保留原配置、项目、任务账本、owner 和 hub ID；以原 hub ID 作为 cluster ID，原来采用该 hub 的节点仍使用原凭据。命令生成独立 peer 身份、私有证书及 `<config>.cluster.json`，随后 `steve run` 自动发现此文件并激活集群应用。输出只有配置路径和公开身份。控制台沿用原来的 token，要求至少 32 个字符，监听地址必须为 loopback；需要调整控制台监听时可在初始化时传 `-ui-address 127.0.0.1:7710`。
