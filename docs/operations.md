...
`prompt_timeout` 到期时，node 托管的一轮（包括 delegate 子任务）会请求节点停止原生命令，节点确认停止后按超时结束；节点无法确认停止时保留执行，须通过原任务恢复入口核实。会话接续观察节点保留的执行时，同样受这个静默超时约束。Agent 给出结果后，这个时钟不再计时，结果的提交和清理不会因为静默超时被打断；node 托管的一轮在结果与超时同时到达时，按已给出的结果完成。
