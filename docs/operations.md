...
重启有两种模式。`now` 沿用原来的行为：工作台不空闲、仍有排队请求或待核实执行时直接失败，保留当前服务。`when-idle` 把请求登记下来，每两秒复核一次，等到服务真正空闲再执行，期间会话照常进行，不会被打断。等待中的请求可以撤销，也可以改为立即重启；同一个 `command_id` 重复提交返回同一次等待，不会排出第二次重启。已被隔离（quarantine）的未结执行不再阻挡等待，否则它会把服务永久钉在旧版本上。
