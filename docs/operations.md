...
导入会复制所选会话的历史并保存导入收据，保留原 native session ID。此时不会发送任务；用户在新会话里发送第一条消息时，Steve 才在独立运行目录恢复它。原历史保持不变，后续 Steve 对话留在 Steve 管理的运行目录中。认证、模型接入、技能和 MCP 使用当前登记的运行配置；原工具的 hooks、插件配置、Botmux 提示词文件和外部聊天集成不会自动启用。dsh 使用 `dsh --profile acp`，支持原生 JSONL 和 Zstandard 历史，仅导入根会话；其子 agent 会话不能单独通过 ACP 恢复。旧版 dsh（例如 `0.1.0-rc.7`）的 ACP 仅支持新会话，需要使用声明支持 resume 的新版工具；所选历史中的图片引用会携带对应附件对象，不会复制其他会话的附件。
