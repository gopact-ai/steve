...
- **函数长度**：非测试函数 > 120 行的清单写在基线文件里，测试断言当前集合 ⊆ 基线；每去掉一个就从基线删掉。阈值随里程碑下调（150 → 120 → 100）。
...
**120 行棘轮（2026-09-09）**。将 `longFunction` 从 150 下调到 120，按职责拆分原先超过 120 行的 23 个函数，最长函数从 149 行降至 120 行。`long_functions.txt` 保持零条，生成器对空集合写零字节文件；临时接口断言和状态字面量清单同样为零条，组合根仍为 670 行 / 15 个内部依赖。HTTP、JSON、配置与日志消息文本不变，没有有意的运行行为偏差。

先固定行为再拆分：RPC 请求边界 golden 覆盖身份、路由、方法、授权、解码与请求体关闭顺序，原有 Raft 状态机回放 golden 不变；账本导入对照覆盖只验证不写入、幂等回放、冲突优先级与事务回滚；回合准备对照覆盖污染、能力漂移、工作区漂移的拒绝顺序及回合锁释放；委派对照覆盖上下文拒绝、成功、失败与令牌存续期。这些新增测试也通过 Go overlay 在 `9adda6a` 的原实现上运行验证。

**下一步**。120 行棘轮落地后，再单独推进 100 行阈值；当前仍有 45 个非测试函数超过 100 行。本轮不改 100 行阈值，真机门禁由主会话执行。
