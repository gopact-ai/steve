...
| Coordinator ↔ node | The coordinator dials out; one token-authenticated TCP connection multiplexes streams (sessions, process streams, artifact operations, files, MCP probes, restarts); on connect the protocol version is negotiated (both sides accept only v2) and the node's advert is exchanged; its feature list names only what a v2 node may lack (`native_history.v1`, `service_restart.v1`). | A node opens one port; a node on another protocol version is refused at the handshake with upgrade advice, and a missing optional capability is reported instead of degrading silently. |
