...
func (a *fleetAdmin) sshService() *sshconnect.Service {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.ssh == nil {
...
func (a *fleetAdmin) SSHDiscover(ctx context.Context) (sshconnect.Discovery, error) {
	return a.sshService().Discover(ctx)
}

func (a *fleetAdmin) SSHCheck(ctx context.Context, alias string) (sshconnect.CheckResult, error) {
	return a.sshService().Check(ctx, alias)
}

func (a *fleetAdmin) SSHPlan(ctx context.Context, req sshconnect.InstallRequest) (sshconnect.InstallPlan, error) {
	return a.sshService().Plan(ctx, req)
}

func (a *fleetAdmin) SSHCommit(ctx context.Context, id string) (sshconnect.InstallResult, error) {
	return a.sshService().Commit(ctx, id)
}

type sshNodeBackend struct{ admin *fleetAdmin }
