...
func desktopAdminFixture(t *testing.T) (*fleetAdmin, string) {
	t.Helper()
	...
	return &fleetAdmin{cfg: cfg, path: install.Paths.Config, catalog: catalog, manager: manager,
		skills: &skills.Live{Map: skillMap, After: func() error { t.Fatal("enrollment restarted existing agents"); return nil }}}, bin
}
