...
const f = { checks: [], plans: [], installs: [], nodeAgentReads: [], errors: [], ready: false, checkError: false, discoveryError: false, reset: false, hold: false, release: null, connected: false, coordinator: "my-desktop", changedNetwork: false };
...
const routeRequest = async (route) => {
    ...
    if (p === "/console/ssh/candidates") return f.discoveryError ? route.fulfill({ status: 503, json: { error: "SSH config is temporarily unavailable" } }) : route.fulfill({ json: { candidates: [candidate], warnings: [{ source: "/test/ssh/config", line: 1, code: "dynamic_include", message: "Some conditional destinations are resolved when connecting." }], revision: "r1" } });
...
try {
    ...
    await dialog.getByRole("textbox", { name: "This machine’s network address", exact: true }).fill(" 10.0.0.4 ");
