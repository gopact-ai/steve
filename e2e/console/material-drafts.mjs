...
const storedQuotes=()=>[...saved].filter(([key])=>key.startsWith('steve.console.draft.quote:')).map(([,value])=>JSON.parse(value)).sort((a,b)=>a.order-b.order).map((entry)=>entry.quote.reply_id);
